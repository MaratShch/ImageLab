...
)
{
    ...
    if (nullptr != paramsHandler)
    {
        ...
        if (nullptr != paramsStrP)
        {
            ...
            PF_CHECKOUT_PARAM(in_data, UnderlyingType(eImageLab2AWB_Controls::AWB2_COLOR_SPACE_POPUP),     current_time, time_step, time_scale, &paramVal[0]);
            PF_CHECKOUT_PARAM(in_data, UnderlyingType(eImageLab2AWB_Controls::AWB2_CHROMATIC_POPUP),       current_time, time_step, time_scale, &paramVal[1]);
            PF_CHECKOUT_PARAM(in_data, UnderlyingType(eImageLab2AWB_Controls::AWB2_ILLUMINATE_POPUP),      current_time, time_step, time_scale, &paramVal[2]);
            PF_CHECKOUT_PARAM(in_data, UnderlyingType(eImageLab2AWB_Controls::AWB2_EXTERME_PIXELS),        current_time, time_step, time_scale, &paramVal[3]);
            PF_CHECKOUT_PARAM(in_data, UnderlyingType(eImageLab2AWB_Controls::AWB2_SATRURATION_THRESHOLD), current_time, time_step, time_scale, &paramVal[4]);
            PF_CHECKOUT_PARAM(in_data, UnderlyingType(eImageLab2AWB_Controls::AWB2_BLACK_LEVEL_THRESHOLD), current_time, time_step, time_scale, &paramVal[5]);

            extra->output->pre_render_data = paramsHandler;

            paramsStrP->colorSpace           = static_cast<eCOLOR_SPACE>(paramVal[0].u.pd.value - 1);
            paramsStrP->chromatic            = static_cast<eChromaticAdaptation>(paramVal[1].u.pd.value - 1);
            paramsStrP->illuminate           = static_cast<eILLUMINATE>(paramVal[2].u.pd.value - 1);
            paramsStrP->percentExtremePixels = static_cast<float>(paramVal[3].u.fs_d.value);
            paramsStrP->saturationThreshold  = static_cast<float>(paramVal[4].u.fs_d.value);
            paramsStrP->blackLevelThreshold  = static_cast<float>(paramVal[5].u.fs_d.value);
            ...
            (in_data->effect_ref,
                UnderlyingType(eImageLab2AWB_Controls::AWB2_INPUT),
                UnderlyingType(eImageLab2AWB_Controls::AWB2_INPUT),
                &req,
...
)
{
    ...
    {
        ERR((extraP->cb->checkout_layer_pixels(in_data->effect_ref, UnderlyingType(eImageLab2AWB_Controls::AWB2_INPUT), &input_worldP)));
        ...
        if (nullptr != input_worldP && nullptr != output_worldP)
        {
            ...
            ERR(extraP->cb->checkin_layer_pixels(in_data->effect_ref, UnderlyingType(eImageLab2AWB_Controls::AWB2_INPUT)));
