...
//
//  Buffer format: BGRA, 32f interleaved (float4: .x=B .y=G .z=R .w=A) -- SAME
//  storage, layout and pitch as the 32f kernel. This is the "16f" path: the
//  buffer is still float32 and the host passes the SAME float* and the SAME
//  pitch (in float4 units) as for 32f. The ONLY difference from the 32f kernel
//  is q16(): every value is rounded to the nearest IEEE binary16 on input and
//  output (q16(v) = __half2float(__float2half(v))), emulating an fp16 pipeline's
//  precision while all accumulation and the eigensolve stay in fp32/fp64. This
//  mirrors the gray-point AWB16 reference exactly. There is NO half4 storage and
//  NO pitch conversion -- the host never needs the __half type.
//
...
#include <cuda_fp16.h>   // __float2half / __half2float for q16()
#include <math.h>
...
namespace {   // internal linkage: avoids symbol clashes with the 32f TU.

// Round an fp32 value to the nearest IEEE binary16-representable value. This is
// the ONLY thing that makes the 16f path differ from the 32f path (matches the
// gray-point AWB16 reference). Storage stays float32; only precision is fp16.
...
{
    r = q16(p.z); g = q16(p.y); b = q16(p.x);   // pin inputs to the half grid
...
#endif
        float4 o;
        o.x = q16(nb); o.y = q16(ng); o.z = q16(nr);   // BGR, pinned to the half grid
        o.w = q16(p.w);                 // alpha quantized + copied through, ALWAYS
