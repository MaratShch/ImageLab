...
PF_Err aeProcessImage_ARGB_4444_16u_CMYK
(
	PF_InData*		in_data,
	PF_OutData*		out_data,
	PF_ParamDef*	params[],
...
) noexcept
{
	...
	for (auto j = 0; j < height; j++)
	{
		...
		for (auto i = 0; i < width; i++)
		{
			...
			localDst[dst_line_idx + i].B = static_cast<A_u_short>(newB * 32768.f);
			localDst[dst_line_idx + i].G = static_cast<A_u_short>(newG * 32768.f);
			localDst[dst_line_idx + i].R = static_cast<A_u_short>(newR * 32768.f);
...
PF_Err aeProcessImage_ARGB_4444_16u_RGB
(
	PF_InData*		in_data,
	PF_OutData*		out_data,
	PF_ParamDef*	params[],
...
) noexcept
{
	...
	for (auto j = 0; j < height; j++)
	{
		...
		for (auto i = 0; i < width; i++)
		{
			...
			/* put to output buffer updated value */
			localDst[dst_line_idx + i].B = CLAMP_VALUE(newB, 0, 32767);
			localDst[dst_line_idx + i].G = CLAMP_VALUE(newG, 0, 32767);
			localDst[dst_line_idx + i].R = CLAMP_VALUE(newR, 0, 32767);
			localDst[dst_line_idx + i].A = A;
