...
) noexcept
{
	...
	for (auto j = 0; j < height; j++)
	{
		...
		for (auto i = 0; i < width; i++)
		{
			...
			localDst[line_idx + i].B = static_cast<A_u_char>(newB * 255.f);
			localDst[line_idx + i].G = static_cast<A_u_char>(newG * 255.f);
			localDst[line_idx + i].R = static_cast<A_u_char>(newR * 255.f);
...
) noexcept
{
	...
	for (auto j = 0; j < height; j++)
	{
		...
		for (auto i = 0; i < width; i++)
		{
			...
			/* put to output buffer updated value */
			localDst[line_idx + i].B = CLAMP_VALUE(newB, 0, 255);
			localDst[line_idx + i].G = CLAMP_VALUE(newG, 0, 255);
			localDst[line_idx + i].R = CLAMP_VALUE(newR, 0, 255);
			localDst[line_idx + i].A = A;
