...
) noexcept
{
	...
	constexpr float sigmoidStep = sigmoidRange / 32768.f;
...
) noexcept
{
	...
	for (j = 0; j < height; j++)
	{
		...
		for (i = 0; i < width; i++)
		{
			...
			const float U = lineSrc[i].R * rgb2yuv[3] + lineSrc[i].G * rgb2yuv[4] + lineSrc[i].B * rgb2yuv[5];
			const float V = lineSrc[i].R * rgb2yuv[6] + lineSrc[i].G * rgb2yuv[7] + lineSrc[i].B * rgb2yuv[8];
			const float Y_modified = static_cast<uint8_t>(FastCompute::sigmoid((static_cast<float>(Y) - 128.f) * sigmoidStep) * 256.f);
