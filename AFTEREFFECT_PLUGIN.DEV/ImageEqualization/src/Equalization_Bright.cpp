...
template <typename T>
inline const T BrightLut (const T& luma, const float& coeff) noexcept
{
	const float fVal = static_cast<float>(luma) / coeff;
	return static_cast<T>(FastCompute::Cbrt(fVal * fVal) * coeff);
}
...
) noexcept
{
	...
	for (j = 0; j < height; j++)
	{
		...
		for (i = 0; i < width; i++)
		{
			lineDst[i].Y = BrightLut (lineSrc[i].Y, coeff);
...
{
	return PR_ImageEq_Bright_VUYA_4444_8u_709(in_data, out_data, params, output);
}
...
{
	return PR_ImageEq_Bright_VUYA_4444_32f_709(in_data, out_data, params, output);
}
...
) noexcept
{
	...
	for (j = 0; j < height; j++)
	{
		...
		for (i = 0; i < width; i++)
		{
			...
			const float lut_Y = BrightLut(Y, coeff);
