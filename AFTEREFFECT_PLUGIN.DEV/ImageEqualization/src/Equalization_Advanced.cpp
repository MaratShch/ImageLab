...
) noexcept
{
	...
	if (-1 != blockId && nullptr != pMemoryBlock)
	{
		constexpr float scale = f32_value_white;
		constexpr float scale2sRGB = 1.f / scale;
		const float* __restrict fReferences = cCOLOR_ILLUMINANT[observer_CIE_1931][color_ILLUMINANT_D65];
...
) noexcept
{
	...
	const auto sizeY = output->height;
	const auto sizeX = output->width;
	...
	if (-1 != blockId && nullptr != pMemoryBlock)
	{
		...
		/* convert from RGB to CIEL*a*b color space */
		const float cs_out_max = ConvertToLabColorSpace(localSrc, srcLabBuffer, cs_out, sizeX, sizeY, src_line_pitch, sizeX, scale2sRGB);

		/* improve */
		fCIELabHueImprove(srcLabBuffer, xyz_buffer, cs_out, sizeX, sizeY, 85.f, lightness_enhanecemnt, cs_out_max);

		/* convert improved XYZ to RGB */
		ImprovedImageRestore(localSrc, localDst, xyz_buffer, sizeX, sizeY, src_line_pitch, sizeX, dst_line_pitch, scale);
...
) noexcept
{
	...
	if (-1 != blockId && nullptr != pMemoryBlock)
	{
		constexpr float scale = static_cast<float>(f32_value_white);
		constexpr float scale2sRGB = 1.f / scale;
		const float* __restrict fReferences = cCOLOR_ILLUMINANT[observer_CIE_1931][color_ILLUMINANT_D65];
