...
(
	const PF_Pixel_VUYA_8u* __restrict inSrc,
	      PF_Pixel_ARGB_8u* __restrict outSrc,
	const uint32_t*         __restrict pLut,
	const int32_t& sizeX,
	const int32_t& sizeY,
	const int32_t& src_line_pitch,
...
) noexcept
{
	...
	{
		const PF_Pixel_VUYA_8u* __restrict lineSrcPtr = inSrc  + j * src_line_pitch;
		      PF_Pixel_ARGB_8u* __restrict lineDstPtr = outSrc + j * dst_line_pitch;
...
) noexcept
{
	...
	for (int32_t j = 0; j < sizeY; j++)
	{
		const PF_Pixel_VUYA_16u* __restrict lineSrcPtr = inSrc  + j * src_line_pitch;
		      PF_Pixel_ARGB_16u* __restrict lineDstPtr = outSrc + j * dst_line_pitch;
...
(
	const PF_Pixel_VUYA_16u* __restrict inSrc,
	      PF_Pixel_BGRA_16u* __restrict outSrc,
	const uint32_t*         __restrict pLut,
	const int32_t& sizeX,
	const int32_t& sizeY,
	const int32_t& src_line_pitch,
...
{
	namespace ColorCoeff = AVX2::ColorConvert::InternalColorConvert;
	constexpr int32_t value_black = static_cast<int32_t>(u16_value_black);
	constexpr int32_t value_white = static_cast<int32_t>(u16_value_white);

	for (int32_t j = 0; j < sizeY; j++)
	{
		const PF_Pixel_VUYA_16u* __restrict lineSrcPtr = inSrc  + j * src_line_pitch;
		      PF_Pixel_BGRA_16u* __restrict lineDstPtr = outSrc + j * dst_line_pitch;
