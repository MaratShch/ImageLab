#!/usr/bin/env python3
"""
Generate 48x48 32-bit RGBA PNG showing a fixed 256-color palette.
Python 3.11 compatible, requires Pillow (`pip install Pillow`).
"""

from PIL import Image

# === Your exact VGA palette (truncated view in comment, full list below) ===
PALETTE_256 = [
...
assert len(PALETTE_256) == 256, f"Palette has {len(PALETTE_256)} entries, expected 256."

def create_palette_image(palette, filename="vga_palette_custom_48x48.png"):
    """
    Renders 256-color palette into 48x48 RGBA image (16x16 grid of 3x3 cells).
    """
    img_w, img_h = 48, 48
    cell_w, cell_h = img_w // 16, img_h // 16  # =3
    ...
    print(f"Saved {filename} ({img_w}x{img_h}, 32-bit RGBA)")

if __name__ == "__main__":
    create_palette_image(PALETTE_256)
