...
{
	const PF_EffectWorld*   __restrict input = reinterpret_cast<const PF_EffectWorld* __restrict>(&params[MEDIAN_FILTER_INPUT]->u.ld);
	uint64_t* __restrict localSrc = reinterpret_cast<uint64_t* __restrict>(input->data);
	...
	constexpr A_long channelMaskL = 0xFFFF0000;
	constexpr A_long channelMaskH = 0xFFFFFFFF;
	...
	switch (kernelSize)
	{
		...
		case 3:
		/* manually optimized variant 3x3 */
			medianResult = AVX2::Median::median_filter_3x3_RGB_4444_16u (localSrc, localDst, height, width, src_pitch, dst_pitch, channelMaskL, channelMaskH);
		break;

		case 5:
		/* manually optimized variant 5x5 */
			medianResult = AVX2::Median::median_filter_5x5_RGB_4444_16u (localSrc, localDst, height, width, src_pitch, dst_pitch, channelMaskL, channelMaskH);
		break;

		case 7:
		/* manually optimized variant 7x7 */
			medianResult = AVX2::Median::median_filter_7x7_RGB_4444_16u (localSrc, localDst, height, width, src_pitch, dst_pitch, channelMaskL, channelMaskH);
		break;

		default:
		/* median via histogramm algo */
			medianResult = median_filter_constant_time_ARGB_4444_16u (localSrc, localDst, height, width, src_pitch, dst_pitch, kernelSize);
		break;
