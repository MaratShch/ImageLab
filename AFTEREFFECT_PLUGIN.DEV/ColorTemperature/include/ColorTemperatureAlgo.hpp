...
inline _tRGB<U> sRGB2LinearRGB (const T& in, const U coeff) noexcept
{
    ...
    */
    ...
    constexpr U reciproc1{ static_cast<U>(1) / static_cast<U>(12.920) };
    constexpr U reciproc2{ static_cast<U>(1) / static_cast<U>(1.0550) };
    _tRGB<U> out;

    const U R{ in.R * coeff };
    const U G{ in.G * coeff };
    const U B{ in.B * coeff };
