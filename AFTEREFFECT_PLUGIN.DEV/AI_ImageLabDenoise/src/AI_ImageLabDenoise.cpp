	...
	PF_LayerDef		*output)
{
	PF_SPRINTF(
		...
		"%s, v%d.%d\r%s",
		strName,
		AI_Denoise_VersionMajor,
		AI_Denoise_VersionMinor,
		strCopyright);
	...
	PF_LayerDef		*output)
{
	...
	out_data->my_version =
		PF_VERSION(
			AI_Denoise_VersionMajor,
			AI_Denoise_VersionMinor,
			AI_Denoise_VersionSub,
			AI_Denoise_VersionStage,
			AI_Denoise_VersionBuild
		);
...
{
    return AI_Denoise_PreRender (in_data, out_data, extra);
}
...
{
    return AI_Denoise_SmartRender(in_data, out_data, extraP);
}
