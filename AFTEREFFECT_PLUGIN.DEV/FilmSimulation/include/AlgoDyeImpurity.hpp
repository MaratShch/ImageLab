...
// ---------------------------------------------------------------------------
//  Stage 12b: Callier's coefficient -- the density a DIRECTIONAL reader sees.
//
//  pR/G/B          density planes, modified IN PLACE
//  sizeX/sizeY     active pixel extent
//  pitch           row stride in ELEMENTS
//  profile         stock being simulated; callier_q lives here
//  scannerSpecular 0 = integrating sphere, 1 = point source / condenser
//
//  IN PLACE, and deliberately so: unlike stage 12 this is a per-channel scaling
//  with no cross-channel mixing, so it needs no second buffer and the
//  retained-buffer policy has nothing to preserve here.
//
//  Placed at the boundary between the developed negative and everything that
//  READS it, because both readers in this chain are affected -- an optical
//  printer with a condenser head and a scanner with a directed source see the
//  same steepened density. BEFORE stage 13, never after: the print stage's own
//  curve has to act on what its optics actually see.
//
//  INERT at scannerSpecular <= 0 and inert at ANY setting on the 93 colour
//  stocks, whose dye images do not scatter. See AlgoCallier.hpp for the law and
//  for why the anchor solve and the print chain's mid-grey reference must both
//  see the same factor. film_sim.callier_density() is the reference.
// ---------------------------------------------------------------------------
