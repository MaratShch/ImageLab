...
has **26 stage entry points**, because nine of the numbered stages carry sub-stages.
...
|---|---|---|---|
| **`film_sim.py`** | the model of record. Every parameter is read here first and every audit re-derives its numbers against this | 26 / 26 stages | the reference for *meaning* |
| **Scalar C++** (`Algo_02..17_Sim.cpp` + 30 headers) | ⚠ **the HIGH-ACCURACY REFERENCE, not a performance target** (project policy, 2026-08-28). Priorities in order: numerical accuracy, clarity, determinism, correctness. Expensive-but-more-accurate mathematics is acceptable here | **26 / 26** stage entry points | reference for *number* |
| **AVX2** (`AVX2/Algo_02..17_Sim.cpp`) | the production path, measured against the scalar build | **26 / 26** stage entry points | complete |
