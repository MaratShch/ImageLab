...
#: Proprietary notice emitted at the head of every generated DATABASE C++ and
#: HPP artefact -- the 26 files listed in build.GENERATED that end in .hpp, .h
#: or .cpp. Owner directive 2026-09-10.
#:
#: ⚠ THE THREE GENERATED .txt FILES DELIBERATELY DO NOT CARRY IT, and that is
#: not an oversight to be tidied up later. `film_names.txt` is consumed as
#: ADJACENT C++ STRING LITERALS pasted into the effect panel's listbox, so a
#: banner there would print this notice into the film dropdown; verify.py
#: asserts the file has no comment banner for exactly that reason.
#: `film_display_order.txt` is bare integers under an identity-permutation
#: guard, and `film_id_migration.txt` was excluded by the same directive.
#:
#: ⚠ IT COSTS SLOT BUDGET. At 1072 bytes x 20 data slots the largest slot moves
#: from 106,886 to about 108,000 against SLOT_SOURCE_LIMIT. That is still
#: feasible at N_DATA_SLOTS = 20, but the headroom is now under 4 kB, so the
#: next prose addition to a profile is the one that forces a repack. The
#: emitter measures the real figure every run -- see the assertion in
#: _distribute.
...
# ---- 20 -> 24, 2026-09-10: the patent and Glafkides harvest ----------------
# ⚠ THIS BUMP IS CAUSED BY REAL NEW DATA, unlike the 19 -> 20 one above, and
# the first attempt at it was caused by something else entirely -- which is
# the part worth recording.
#
# WHAT ACTUALLY GREW. The schema-v29 harvest populated `EmulsionSpec` on
# essentially the whole database: `base_um` on 180 stocks (was 19),
# `grain_um` / `iodide_mol_pct` / `size_sigma_log` on 170, `coated_um` on 92,
# `antihalation` on 28, plus a `ProcessingFamily` rate law on 4 and 17 new
# ParamSource records. Every one of those needs a citation, and
# `EmulsionSpec.source` is mandatory as soon as any field on the record is
# set, so 165 profiles that carried an empty emulsion record now carry a
# cited one. Emitted total: 1 999 173 -> 2 249 861 bytes, i.e. +250 688.
#
# ⚠ AND THE FIRST DRAFT WAS 2 513 004 BYTES, OF WHICH 309 116 WAS THE SAME
# PROSE REPEATED. The harvest pass had written its full reasoning -- a
# 2 403-byte argument about support gauges, a 1 000-byte derivation per
# crystal class, a 900-byte evidence-class note -- into the `source` FIELD of
# every profile it touched: 26 identical copies of one essay, 170 of another.
# The slot packer refused it outright (no partition into 20 files of 112 000
# bytes exists at that size) and that refusal was correct for a better reason
# than size: a generated database is a SHIPPED ARTEFACT and the emitted
# string should be a CITATION, while the argument for it belongs in a Python
# comment that no consumer pays for. The four templates were split that way
# -- full reasoning preserved verbatim in `#` comments beside them, compact
# citation emitted -- which recovered 263 kB, and the +250 kB that remains is
# per-stock data that genuinely differs from stock to stock.
#
# WHY 24 AND NOT 22. Measured on the identical 2 249 861 bytes:
#       22 slots : minimum feasible maximum 111 777   OK, headroom     223
#       23 slots :                          107 542   OK, headroom   4 458
#       24 slots :                          103 184   OK, headroom   8 816
# Twenty-two is arithmetically feasible and practically useless -- 223 bytes
# is less than one citation, so the next stock added would break the build
# again immediately and cost another manual .vcxproj edit. Twenty-four gives
# 8 816 bytes of headroom, which is the same order as the 6 750 the 19 -> 20
# bump chose and deliberately so.
#
# ⚠ **`film_profiles_data_21.cpp` THROUGH `_24.cpp` MUST BE ADDED TO THE
# VS2015 .vcxproj BY HAND.** Nothing in this repository can do it. If they
# are missed the plugin fails at LINK time with an unresolved
# `AppendFilmProfiles_21`..`_24` -- a loud failure, never a wrong render or a
# shifted film index. CMake globs and needs no change.
# ⚠ AND THIS DELIVERY CARRIES SIX SUCH STEPS, not four: `_19.cpp` from
# 2026-09-06f and `_20.cpp` from 2026-09-08 were never added either. All six
# files need the same one-time edit.
...
ENUM_TEMPLATE = COPYRIGHT_NOTICE + """// Generated by cpp_codegen.py on @GENERATED@ -- DO NOT EDIT BY HAND.
