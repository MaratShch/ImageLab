...
>
> ✅ **`FilmMtfResponse` WAS BYPASSED UNTIL 2026-09-03 AND IS NOT ANY MORE.** The measured
> `1/(1+(f/f50)^q)` rolloff is a frequency-domain form and **the C++ side has no FFT at all** --
> `AlgoSeparableBlur.hpp` opens by arguing why not -- so it rendered on the legacy Gaussian: exact
> at f50 by construction, **up to 3.8x too much modulation at 2x f50**. ⚠ The entry recording that
> also went stale, saying **9 stocks** where the database carries **22**. Closed by `FilmMtfKernel`,
> a two-Gaussian separable fit keyed on the exact stored `q`, convolved as two more lobes of the
> multi-Gaussian blur stage 6 already ran. No FFT was added. Worst max|error| **0.0384** against
> **0.1737** for the Gaussian it replaced.
>
...
⚠ **RE-DERIVED FROM THE DRIVER, 2026-08-31.** This table read 15 rows for weeks and the pipeline
has **27 stage entry points**, because ten of the numbered stages carry sub-stages (9 now carries three).

⚠ **AND THE FIRST RE-DERIVATION SAID 25, WHICH IS THE SAME CLASS OF ERROR ONE LINE LATER.** It
grepped `AlgoStage[0-9]+[a-c]?_` across `Algo_*_Sim.cpp` and missed **12b Callier**, whose body is
an inline function in `AlgoCallier.hpp` and therefore has no stage translation unit to grep — while
the running engine prints `12b  Callier` in its per-stage profile every frame. The count is now
taken from **`AlgorithmMain.cpp`**, the one file that must call every stage exactly once, and
`doc_consistency.py` registers it so a repeat fails the build.
...
| 9c | `AlgoStage09c_BromideDrag` | the processing MACHINE's directional restraint: bromide released by development is dragged along the transport axis and keeps restraining where it lands, leaving a one-sided streak behind every dense region (queue C23). ⚠ The only stage that belongs to the LAB rather than to the film, so it reads `ProcessingSpec` and not `FilmProfile`'s own fields — and the only one that runs IN PLACE besides 12b. ⚠ **Inert on all 176 stocks**: no document in the corpus quantifies a bromide gradient, so it costs one branch (3 ns/frame scalar, 7 ns AVX2). Active cost, single thread at 2048x1152: **11.42 ms/Mpx scalar, 2.93 ms/Mpx AVX2** — bandwidth-bound at four passes, not arithmetic-bound at one fmadd per pixel | density |
...
| | ⚠ *(stage 11, added 2026-09-11c)* | **`GrainSpec.anisotropy` is now applied by BOTH C++ engines**, through the new anisotropic separable blur `AlgoGaussianBlurPlaneWrapXY`, where **σ_y = a · σ_x is the spatial-domain spelling of the reference’s frequency-domain stretch**. ⚠⚠ **Until 2026-09-11c the field was emitted into C++ and read by NEITHER engine while the Python reference DID read it — 32 of 184 stocks carry 1.02–1.10.** ⚠ **And the reference was leaking the SAME stretch into six stages that are not grain** — veiling flare, halation, emulsion MTF, the DIR coupler blur, scan MTF, duplication MTF and the réseau — because **one `FreqGrid` object was shared by everything**; Python now builds an isotropic `grid` for the optical stages and a separate `grain_grid` used only by the camera negative’s grain. ⚠ **Print and dupe grain deliberately get 1.0 in all three implementations: different emulsions, different factories, no measurement** | density |
| 12 | `AlgoStage12_DyeImpurity` | dye impurity / scanner crosstalk matrix | density |
| 12b | `AlgoStage12b_Callier` | the density the reader's optics see — diffuse vs specular (queue C41). ⚠ Its body is inline in `AlgoCallier.hpp`, not in a stage `.cpp`; it runs in place on the stage-12 planes and costs ~0 when the coefficient is unity | density |
| 13 | `AlgoStage13_Duplication` | duplication generations, then the release print | density |
| | ⚠⚠ *(stage 13, corrected 2026-09-11d)* | **The AVX2 path evaluated the print curve with a FAST APPROXIMATE SOFTPLUS — the Schraudolph exp plus a short log — where the scalar twin calls the exact `AlgoSoftplus`, and its own comment called that a DOCUMENTED MODE DIFFERENCE worth "2.4e-03 to 3.2e-03 in plane MEAN".** ⚠⚠ **A PLANE MEAN IS AN INTEGRAL AND THE ERROR IS SIGNED, so most of it cancelled before anybody looked. Measured PER PIXEL by the new `stage_parity.py`: 1.03e-02 to 1.37e-02 D, four to five times the quoted figure, reaching the screen as 7.4e-03 to 7.7e-03 of transmittance — ABOUT TWO EIGHT-BIT CODE VALUES ON EVERY STOCK THAT PRINTS.** ⚠ **Stages 14–17 inherited it and added nothing of their own**, proved by the two reversal stocks holding the float32 floor through the same range. Fixed with an accurate exp and log **defined in stage 13's own translation unit, not in the shared math header** (a Common component shipped in a different archive — the `Exp2Accurate` integration lesson of the same day): **5.90e-06 D worst over the whole operating domain, a factor of 2300.** ⚠ **One subtlety is recorded in the code: the log's mantissa must be decomposed into [0.707, 1.414], not the [1, 2) the raw exponent field gives, because the polynomial is a minimax fit for log(1+f) on f ∈ [−0.293, 0.414]; a first version that skipped that measured 1.38e-02 instead of 2.84e-07** | density |
...
| | ⚠⚠ *(stage 14, corrected 2026-09-11c)* | **The AVX2 path evaluated 10⁻ᵈ with the raw Schraudolph bit-hack until 2026-09-11c**: **2.98 % relative error = 5.57 eight-bit code values**, and **0.978161 at D = 0**, so **the vector build rendered its WHITE POINT 2.2 % LOW** — while the tMin/tMax normalisation anchors beside it used exact `std::pow`, making the vector path internally inconsistent, and **stage 17 clips at 1.0, so the error MOVED WHERE HIGHLIGHTS CLIP.** Replaced with `Exp2Accurate` (degree-5 minimax plus exact exponent assembly), **0.000326 % worst relative error and exactly 1.0 at D = 0** | → display linear |
...
| | ⚠ *(stage 17, corrected 2026-09-11c)* | **The AVX2 clamp mapped a NaN to 1.0 — WHITE — while the scalar `CLAMP_VALUE` passed the NaN through unchanged. Both now map NaN to ZERO.** ⚠⚠ **The AVX2 comment had always CLAIMED it mapped to the low bound; it did the opposite**, which is why nobody re-read the line | — |
...
|---|---|---|---|
| **`film_sim.py`** | the model of record. Every parameter is read here first and every audit re-derives its numbers against this | ⚠⚠ **23 / 27 stages — CORRECTED 2026-09-11c; this cell read "27 / 27" and was wrong.** Python has **no `03c_TemporalFlicker`, no `09b_NegativeDefects`, no `15_GateWeave` and no `16_GateDefects`**, because it is a **single-frame reference** and three of the four are frame-to-frame effects. ⚠ **The consequence is larger than the missing rows: those four stages have NO REFERENCE IMPLEMENTATION TO VALIDATE AGAINST at all** | the reference for *meaning*, on the 23 stages it has |
| **Scalar C++** (`Algo_02..17_Sim.cpp` + 30 headers) | ⚠ **the HIGH-ACCURACY REFERENCE, not a performance target** (project policy, 2026-08-28). Priorities in order: numerical accuracy, clarity, determinism, correctness. Expensive-but-more-accurate mathematics is acceptable here | **27 / 27** stage entry points | reference for *number* |
| **AVX2** (`AVX2/Algo_02..17_Sim.cpp`) | the production path, measured against the scalar build | **27 / 27** stage entry points | complete |

⚠⚠ **WHAT "MEASURED AGAINST THE SCALAR BUILD" ACTUALLY COVERS, STATED 2026-09-11c BECAUSE THE ABSENCE OF THIS SENTENCE COST A MONTH.** The two C++ paths **share one driver**, `AlgorithmMain.cpp`, so **stage ORDER is identical by construction** and cannot drift, and the 2026-09-11c audit found **zero per-stage database-field asymmetry between them across all 27 stages**. ⚠⚠ **BUT NUMERIC SCALAR-VERSUS-AVX2 TWIN CHECKING EXISTS FOR STAGES 08b, 09 AND 09c ONLY. THE OTHER 24 HAVE NO AUTOMATED NUMERIC TWIN CHECK.** That is a hole in COVERAGE, not in tolerance, and it is how three real behavioural differences survived to 2026-09-11c: AVX2 stage 14 evaluated **10⁻ᵈ** with the raw Schraudolph bit-hack at **2.98 % relative error**, returning **0.978161 at D = 0** so the vector build rendered its **white point 2.2 % low** while its own tMin/tMax anchors used exact `std::pow`; **`GrainSpec.anisotropy` was emitted into C++ and read by NEITHER engine** on the **32 of 184 stocks** carrying 1.02–1.10; and the **stage 17 final clamp mapped NaN to 1.0 — WHITE — in AVX2** where the scalar twin passed it through. All three are fixed, and **a v33 render is NOT bit-identical to a v32 one.** ⚠ **Until the other 24 stages gain a numeric twin check, "measured against the scalar build" means measured on three stages.**

⚠⚠ **AND THEY GAINED ONE THE NEXT DAY. CORRECTED 2026-09-11d: "MEASURED AGAINST THE SCALAR BUILD" NOW MEANS MEASURED ON ALL 27 STAGES, PER PIXEL.** `stage_parity.py` compiles both trees, renders with `ALGO_RETAIN_ALL_STAGES=1` and compares **all 25 retained planes** — 25 triples cover all 27 stages, because 9c writes S09b in place and 12b writes S12 in place — by **per-pixel max absolute difference** on five stocks chosen to exercise different paths, at a frame size deliberately not a multiple of 8 so the AVX2 tail mask runs on every row, with grain and damage on. ⚠⚠ **ON ITS FIRST RUN IT FOUND A LIVE DEFECT THAT HAD BEEN DOCUMENTED AS ACCEPTABLE SINCE 2026-08-28: AVX2 stage 13, the print curve, kept a fast approximate softplus on the Schraudolph exp and a short log where the scalar twin calls the exact `AlgoSoftplus`.** Its own comment priced that at **"2.4e-03 to 3.2e-03 in plane MEAN"** — ⚠ **a plane mean is an integral and the error is signed, so most of it cancelled before anybody looked.** Per pixel it is **1.03e-02 to 1.37e-02 D, four to five times the quoted figure**, arriving at the screen as **7.4e-03 to 7.7e-03 of transmittance — about TWO EIGHT-BIT CODE VALUES on every stock that prints**; **stages 14–17 inherited it and added nothing of their own**, which the two reversal stocks prove by holding the float32 floor through the same range. ⚠ **Fixed with an accurate exp and log defined in stage 13's OWN translation unit and not in the shared math header** — that header is a Common component shipped in a different archive, the lesson of the `Exp2Accurate` integration failure the same day — measured after at **5.90e-06 D worst over the whole operating domain, a factor of 2300.** ⚠⚠ **ALL 125 RETAINED PLANES ACROSS FIVE STOCKS NOW AGREE INSIDE THE STRICT float32 BUDGET WITH ZERO PINNED EXCEPTIONS; the audit's `KNOWN_DIVERGENCE` table shipped with 21 entries and is EMPTY; worst disagreement where both twins run the same algorithm is 3.189e-05.** ⚠ **The paragraph above is kept unrewritten because the hole it names is what this audit was built to fill, and because its closing sentence is the best short statement of why a coverage gap outranks a tolerance argument.**
...
| `bromide_parity.py` | stage 9c, Python against BOTH the scalar and the AVX2 build. ⚠ Injects a record into a COPY of a real profile, because the stage is inert on every shipped stock and an untested inert stage is how three engines drift apart unnoticed | worst **4.87e-07** scalar (tolerance 2e-06 at `sizeof(AlgoType) == 8`) and **4.77e-07** AVX2 (tolerance 2e-03), over 2 stocks × 4 cases × 2 pixel scales × 5883 values. Also asserts the streak trails on the side `direction` names, and that one `length_mm` renders the same physical streak at 25 and 100 px/mm to **2.9e-04** of the removed fraction |
| `spectral_mono_parity.py` | the monochrome collapse weights, both engines | **68/68 stocks agree exactly, zero guard gaps** |
| `stage_parity.py` | ⚠⚠ **NEW 2026-09-11d, AND IT IS THE AUDIT THAT CLOSES THE COVERAGE HOLE THE PARAGRAPH ABOVE DESCRIBES.** Every stage, scalar against AVX2: it compiles **both** trees, renders with `ALGO_RETAIN_ALL_STAGES=1`, dumps **all 25 retained planes** as raw float64 — **25 triples cover all 27 stages**, because 9c writes S09b in place and 12b writes S12 in place — and compares by **PER-PIXEL MAX ABSOLUTE DIFFERENCE**, never by a plane mean. Five stocks chosen to exercise different paths: a colour negative that prints, a reversal stock that does **not** print, a monochrome, ⚠ **the only stock in the database with an active réseau**, and the cine negative with the strongest couplers. ⚠ **44 px frames, deliberately not a multiple of 8, so the AVX2 tail mask runs on every row**; grain and damage **ON**; tolerance **512 · 2⁻²⁴ · max(1, \|plane\|)**, with **a factor of ln 10 on the transmittance stages** because T = 10⁻ᵈ has derivative −ln10·T. ⚠⚠ **It SKIPS only when there is no AVX2 tree; with both trees present a disagreeing stage is a FAIL, deliberately — a `[SKIP]` line in an otherwise green log is what hid an earlier defect** | **all 125 retained planes over 5 stocks inside the STRICT float32 budget, with ZERO pinned exceptions.** Its `KNOWN_DIVERGENCE` table shipped with **21 entries and is now EMPTY**. Worst disagreement on any stage where both twins run the same algorithm **3.189e-05**. Runs in **42 s**, wired in after `interimage_parity` |
| `verify.py` | 691 passing model and data assertions beside the one baselined FAIL ⚠ **re-counted 2026-09-11c from the v33 build, where this cell read 462 and had drifted by 229 — then re-read 2026-09-11d off the live build log, because `G-ENGINE-INCLUDES` took the suite from 690 to 691 the same afternoon** | **691 PASS / 1 FAIL** — see below |
...
- ✅ ~~**`FilmMtfResponse` has no stage caller in C++.**~~ **Closed 2026-09-03.** Both twins now
  convolve `FilmMtfKernel`, a two-Gaussian separable fit of the rolloff, and `FilmMtfResponse` left
  `cpp_parity.LAW_BYPASS_BASELINE` because it gained a caller. ⚠ **What remains is an
  APPROXIMATION with a stated bound, not a divergence**: Python applies the exact law, C++ the
  fitted kernel, and the two differ by at most **0.0384** in modulation on the 22 measured stocks.
  `RenderSettings.mtf_use_kernel` renders Python through the same kernel when the two must be
  compared on identical arithmetic.
...
- ✅ ~~⚠⚠ **NEW 2026-09-11c — AND IT IS THE GAP THAT LET THE OTHER THREE DEFECTS OF THAT DAY STAND: only 3 of the 27 stages have a numeric scalar-versus-AVX2 twin check.** Stages **08b, 09 and 09c** are compared numerically on every build; **the other 24 are not compared at all.**~~ **CLOSED 2026-09-11d by `stage_parity.py`, ONE DAY OLD WHEN IT WAS CLOSED, AND IT PAID ON THE FIRST RUN.** All **27 stages** are now compared **per pixel** through the **25 retained planes** (9c writes S09b in place, 12b writes S12 in place), on five stocks, at a frame size deliberately not a multiple of 8 so the AVX2 tail mask runs. **All 125 planes agree inside the strict float32 budget with ZERO pinned exceptions; the `KNOWN_DIVERGENCE` table shipped with 21 entries and is empty; worst same-algorithm disagreement 3.189e-05.** ⚠ **What it found first was AVX2 stage 13's fast approximate softplus — 1.03e-02 to 1.37e-02 D per pixel, about two eight-bit code values on every stock that prints, against a "2.4e-03 in plane mean" that had licensed leaving it alone.** Stage order still cannot drift (one shared driver) and the 2026-09-11c audit still reports **zero per-stage database-field asymmetry across all 27**, so all three axes — order, fields and numbers — are now gated.
- ⚠⚠ **NEW 2026-09-11c — four stages have NO REFERENCE IMPLEMENTATION AT ALL.** Python is the model of record and implements **23 of the 27**; it has no **`03c_TemporalFlicker`**, **`09b_NegativeDefects`**, **`15_GateWeave`** or **`16_GateDefects`**, because it is a single-frame reference and three of the four are frame-to-frame effects. ⚠ **Those four cannot be validated against anything — not a tolerance question, an absence of a comparand.**
...
| `film_profiles.py` | Physical parameters, **184 film stocks, 11 print stocks**, 14 gauges, **schema v33** (re-derived 2026-09-11c; this cell read "176 stocks, schema v27" until then. ⚠ **`PROJECT_STATE.md` is generated from the live module on every build and is the authority for these counts** — the `SCHEMA_VERSION` history log at the top of the file is the authority for why each version exists, and it records why the constant sat four versions stale until 2026-08-31; the `SCHEMA_VERSION` history log at the top of the file is the authority, and it records why the constant sat four versions stale until then). Holds the ONE definition of both the grain-σ law (`grain_sigma`) and the MTF law (`mtf_response`), mirrored in the generated C++ as `FilmGrainSigma` / `FilmMtfResponse`. ⚠ `FilmMtfResponse` is reached in C++ through `FilmMtfKernel`, its fitted separable equivalent — there is still no FFT in the plugin tree; see *Implementation status* |
...
| `png_compare.py` | ⚠ **NEW 2026-09-11c.** Standalone PNG comparison utility at the generator root — **Pillow + NumPy only, no OpenCV**, Python 3.12. It exists because **v33 is the first schema version whose renders are NOT bit-identical to its predecessor’s** (three engine defects were fixed on the render path), so a way to look at WHAT moved stopped being optional. Not part of the build chain and not registered as an audit |
...
| `dye_density.py` | Audit: re-derives the **12 adopted spectral dye-density sets in the Kodak H-1 layout** from the sheets' vector paths; 5285 and 2383 are the validation pair. ⚠ Its docstring records the three defects that had put 7239/5217/5218 on a "failed" list when the sources were fine, and the measured refusal of 5246. The Fuji and Konica house styles are `fuji_konica_dye.py` below. ⚠ **Its `ticks()` docstring now records the OVERBAR MINUS** — Kodak's spectral panels draw negative tick labels with an overbar the PDF text layer does not encode, so the harvester silently fits the top half of the axis; `signed_ticks()` reconstructs the sign from position and refuses rather than guesses |
| `fuji_konica_dye.py` | Audit: the **7 dye sets adopted 2026-09-04** from the FUJI and KONICA house styles, which `dye_density.py` cannot read — three **raster** panels (PROVIA 100F by process ink, SENSIA 100 and VELVIA 50 by slope-predictive tracking), two vector panels with no rotated axis caption (the Konica pair), one whose three dyes are a single path of three subpaths (PROVIA 400X), and VISION3 200T 5213, whose Kodak layout still needed the frame chosen by **tick count** because three curve bounding boxes sit closer to the caption than the plot does. ⚠ It also asserts the **two shared drawings** — PROVIA 100F = SENSIA 100, and the Konica pair's magenta and cyan — with negative controls, so seven profiles cannot start reading as seven independent measurements. **Extended 2026-09-04c**: the two **raster** VISION3 panels 5203 and 5207 — whose recorded blocker was that they are raster and whose real one was a **dashed D-min** — read by a tick-comb calibration matched to an asserted ladder, with the neutral+D-min pair refused on both and the family-C identity run over the band the dashes do cover; and the **six neutral + D-min panels**, each gated on the **orange mask falling towards the red** and all six asserted to be six drawings rather than fewer |
...
| `stage_parity.py` | **Audit, NEW 2026-09-11d: EVERY STAGE, SCALAR AGAINST AVX2, PER PIXEL — the audit that ended the era in which three of twenty-seven stages were compared and twenty-four were not.** It compiles **both** trees, renders with `ALGO_RETAIN_ALL_STAGES=1`, dumps **all 25 retained planes** as raw float64 (25 triples cover all 27 stages — 9c writes S09b in place, 12b writes S12 in place) and takes the **per-pixel max absolute difference**. Five stocks chosen for the paths they exercise, not their names; **44 px frames, deliberately not a multiple of 8, so the AVX2 tail mask runs on every row**; grain and damage on; tolerance **512 · 2⁻²⁴ · max(1, \|plane\|)** with **a factor of ln 10 on the transmittance stages**, because T = 10⁻ᵈ has derivative −ln10·T. ⚠⚠ **A DISAGREEMENT IS A FAIL, NOT A NOTE, whenever both trees are present; it SKIPS only when there is no AVX2 tree at all — because a `[SKIP]` line in a green log is exactly what hid an earlier defect.** ⚠ **Its first run found the AVX2 stage-13 print curve running an approximate softplus at two eight-bit code values per pixel, documented since 2026-08-28 as an acceptable mode difference on the strength of a PLANE MEAN.** Now: all 125 planes inside the strict float32 budget, **`KNOWN_DIVERGENCE` empty where it shipped with 21 entries**, worst same-algorithm disagreement **3.189e-05**, 42 s |
| `verify.py` | **691 PASS / 1 FAIL by design — the saturation-hierarchy baseline** ⚠ *(re-counted 2026-09-11c from the v33 build, where this cell read 510/509; re-read 2026-09-11d off the live build log, because `G-ENGINE-INCLUDES` took the suite from 690 PASS to 691 the same afternoon)*: curves, calibration, anchors, isotropy, PNG, flare, generations, réseau, spectral data, provenance guards, the grain LEVEL contract (amplitude = stored rms at net density 1.0, all 465 stock-channels, plus an empirical aperture-integrated end-to-end check), edge cases. Render-heavy. ⚠ **Slicing is currently broken for any slice that does not start at 1** — a later section references a name bound in an earlier one (`NameError: _fpm`). Full runs are unaffected |
...
| `gevachrome_1968_raster.py` | Audit (queue G2): all four Rens & Van Bets 1968 raster plot sets. ⚠ Its blocker was never resolution — the page **curls**, so one fitted log scale puts 100 c/mm off the panel; **piecewise interpolation between the nine printed gridlines** fixed it on the 115 ppi scan already on file |
| `sayanagi_callier.py` | Audit (queues C43, C44): Sayanagi 1959's Q from grain optics. ⚠ His Q_II contains **no density**, which is why C44 closed with a null code change; refitting Mees FIG. 179 with the base modelled turned `callier_q` from a class constant into β(γ) |
| `ooue_1959_granularity.py` | Audit (queues J1, J2): Ooue 1959 Parts 1 and 2 — a measured Wiener spectrum, an autocorrelation, and the sentence that settled this corpus's rms/mean-square ambiguity in the author's own words |
| `takano_1969_granularity.py` | Audit (queues TK1–TK5): Takano 1969. ⚠ **The first measurement this project's aperture term was ever checked against**, and it fits; supplies the five-point clump census C45 was missing, and adopts σ(D)↔σ(T) to fourth order as an inert helper |
| `sehlin_kennel_1985.py` | Audit (queue E5): Sehlin & Kennel, *SMPTE Journal* July 1985. ⚠ A good σ(D) trace that was **withdrawn** — its anchors are the figure's plotted density where `sigma_anchors` reads per-layer analytical density, and `cpp_parity` caught it. Level and Fig. 12's f50 refused too |
| `fuji_neopan_ss.py` | Audit (queue N1): FUJIFILM AF3-411E(N), the sheet that turned §23k.8's refusal into `FUJI_NEOPAN_SS`. ⚠ Its §9 **prints the average gradient on every curve**, so the trace is self-checking; two calibration traps and the two shallowest curves are recorded rather than smoothed over |
...
**Re-counted 2026-09-02.** `python verify.py` runs a **510-check suite: 509 PASS and 1 FAIL by
...
output**. ⚠ **ADDED 2026-09-11d and not in the list above because it did not exist when the list was
written: `stage_parity`, all 25 retained planes over 5 stocks per pixel, ZERO pinned exceptions.**
...
plugin's own `Algo_08_Sim.cpp` / `Algo_09_Sim.cpp` (`interimage_parity.py`). ⚠ **And since
2026-09-11d, EVERY STAGE of both C++ builds against each other, per pixel, through the 25 retained
planes (`stage_parity.py`) — which is a different question from the four laws above: the laws ask
whether the mathematics agrees, and this asks whether the two compiled paths agree, which is where
all four of the defects found on 2026-09-11c and 2026-09-11d actually lived.**
