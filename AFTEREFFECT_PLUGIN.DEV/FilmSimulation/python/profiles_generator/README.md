...
| `verify.py` | 691 model and data assertions ⚠ **re-counted 2026-09-11c from the v33 build; this cell read 462 and had drifted by 229** | **690 PASS / 1 FAIL** — see below |
...
- ⚠⚠ **NEW 2026-09-11c — AND IT IS THE GAP THAT LET THE OTHER THREE DEFECTS OF THAT DAY STAND: only 3 of the 27 stages have a numeric scalar-versus-AVX2 twin check.** Stages **08b, 09 and 09c** are compared numerically on every build; **the other 24 are not compared at all.** Stage order cannot drift — the two paths share `AlgorithmMain.cpp` — and the same audit found **zero per-stage database-field asymmetry across all 27**, so what is missing is specifically the **numeric** check. ⚠ A 2.98 % error in stage 14 would have failed on the first build if any twin check had covered it.
...
| `verify.py` | **691-check suite, 690 PASS / 1 FAIL by design — the saturation-hierarchy baseline** ⚠ *(re-counted 2026-09-11c from the v33 build; this cell read 510/509)*: curves, calibration, anchors, isotropy, PNG, flare, generations, réseau, spectral data, provenance guards, the grain LEVEL contract (amplitude = stored rms at net density 1.0, all 465 stock-channels, plus an empirical aperture-integrated end-to-end check), edge cases. Render-heavy. ⚠ **Slicing is currently broken for any slice that does not start at 1** — a later section references a name bound in an earlier one (`NameError: _fpm`). Full runs are unaffected |
