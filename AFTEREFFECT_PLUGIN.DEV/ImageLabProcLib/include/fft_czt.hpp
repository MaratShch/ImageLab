...
template <typename T>
inline void FFT_Pow2_Helper (ptrdiff_t N, T* data, bool inverse) noexcept
{
    if (N <= 1) return;
...
// ----------------------------------------------------------------------------
template <typename T>
inline void Execute_FFT_Pow2 (ptrdiff_t M, T* data) noexcept
{
...
inline void Execute_IFFT_Pow2 (int32_t M, T* data) noexcept
{
    ...
    for (ptrdiff_t i = 0; i < 2 * M; ++i) data[i] *= invM;
}
