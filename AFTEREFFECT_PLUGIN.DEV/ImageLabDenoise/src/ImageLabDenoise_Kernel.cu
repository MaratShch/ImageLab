...
(
    ...
    int width, int height, int max_patches
)
{
    int ref_x = blockIdx.x; 
    int ref_y = blockIdx.y;
    int tx = threadIdx.x; 
    int ty = threadIdx.y; 
    int tid = ty * 16 + tx;

    int cand_x = ref_x + tx - 8;
    int cand_y = ref_y + ty - 8;
    ...
    for (int r = 0; r < 8; ++r) 
    {
        for (int c = 0; c < 8; ++c) 
        {
            float pRef  = tex2D<float>(tex_Y, ref_x + c, ref_y + r);
    ...
    __shared__ float smem_ssd[256];
    __shared__ int smem_cx[256];
    __shared__ int smem_cy[256];
    ...
    if (tid == 0) 
    {
        ...
        for (int k = 0; k < max_patches; ++k) 
        {
            ...
            for (int i = 0; i < 256; ++i) 
            {
                if (smem_ssd[i] < min_ssd) 
                {
                    min_ssd = smem_ssd[i];
                    min_idx = i;
                }
            }
            searchPool[pool_offset + k] = { smem_cx[min_idx], smem_cy[min_idx] };
            smem_ssd[min_idx] = 1e30f;
        }
    }
}
...
(
    ...
    float* __restrict__ accum_weight, int width, int height, int max_patches
)
{
    int ref_x = blockIdx.x;
    int ref_y = blockIdx.y;
...
)
{
    ...
    // --- 2. SPATIAL DOWNSAMPLING (PYRAMID) ---
    ...
    dim3 searchBlocks(width, height);
    dim3 searchThreads(16, 16);
    int max_patches_per_group = 32; 

    Kernel_L2_BlockMatching<<<searchBlocks, searchThreads, 0, stream>>>
        (tex_Y, gpuMem.d_SearchPool, width, height, max_patches_per_group);

    cudaDestroyTextureObject(tex_Y);
    
    // --- 5. COLLABORATIVE BAYES & NORMALIZATION (Y, U, V) ---
    ...
    dim3 bayesBlocks(width, height);
    dim3 bayesThreads(8, 8); 
    size_t sharedMemBytes = max_patches_per_group * 64 * sizeof(float);
    ...
    Kernel_CollaborativeBayes_Aggregate<<<bayesBlocks, bayesThreads, sharedMemBytes, stream>>>
        (gpuMem.d_Y_planar, gpuMem.d_SearchPool, gpuMem.d_NoiseCov_Y, gpuMem.d_Accum_Y, gpuMem.d_Weight_Count, width, height, max_patches_per_group);
    Kernel_Normalize_Accumulators<<<normBlocks, normThreads, 0, stream>>>
        (gpuMem.d_Accum_Y, gpuMem.d_Weight_Count, width, height);

    cudaMemsetAsync(gpuMem.d_Weight_Count, 0, width * height * sizeof(float), stream);
    Kernel_CollaborativeBayes_Aggregate<<<bayesBlocks, bayesThreads, sharedMemBytes, stream>>>
        (gpuMem.d_U_planar, gpuMem.d_SearchPool, gpuMem.d_NoiseCov_U, gpuMem.d_Accum_U, gpuMem.d_Weight_Count, width, height, max_patches_per_group);
    Kernel_Normalize_Accumulators<<<normBlocks, normThreads, 0, stream>>>
        (gpuMem.d_Accum_U, gpuMem.d_Weight_Count, width, height);

    cudaMemsetAsync(gpuMem.d_Weight_Count, 0, width * height * sizeof(float), stream);
    Kernel_CollaborativeBayes_Aggregate<<<bayesBlocks, bayesThreads, sharedMemBytes, stream>>>
        (gpuMem.d_V_planar, gpuMem.d_SearchPool, gpuMem.d_NoiseCov_V, gpuMem.d_Accum_V, gpuMem.d_Weight_Count, width, height, max_patches_per_group);
    Kernel_Normalize_Accumulators<<<normBlocks, normThreads, 0, stream>>>
        (gpuMem.d_Accum_V, gpuMem.d_Weight_Count, width, height);

    // --- 6. BACKWARD COLOR CONVERSION ---
