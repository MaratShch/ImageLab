...
) noexcept
{
    ...
    for (int32_t y = 0; y < h; ++y)
    {
        ...
        const PF_Pixel_ARGB_16u* pIn_row = pInput + (y * src_pitch);
        PF_Pixel_ARGB_16u* pOut_row = pOutput + (y * dst_pitch);
        ...
        for (; x < w; ++x)
        {
            ...
            PF_Pixel_ARGB_16u* pOut = pOut_row + x;
            const PF_Pixel_ARGB_16u* pIn = pIn_row + x;
