...
) noexcept
{
    ...
    for (int32_t y = 0; y < h; ++y)
    {
        ...
        const PF_Pixel_BGRA_32f* pIn_row = pInput + (y * src_pitch);
        PF_Pixel_BGRA_32f* pOut_row = pOutput + (y * dst_pitch);
        ...
        for (; x < endX; x += vecSize)
        {
            ...
            // the 32-bit registers back into the 128-bit BGRA_32f struct layout
        ...
        for (; x < w; ++x)
        {
            ...
            PF_Pixel_BGRA_32f* pOut = pOut_row + x;
            const PF_Pixel_BGRA_32f* pIn = pIn_row + x;
