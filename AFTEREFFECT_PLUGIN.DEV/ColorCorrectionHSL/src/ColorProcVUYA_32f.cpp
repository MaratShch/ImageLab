...
) noexcept
{
	...
	for (auto j = 0; j < height; j++)
	{
		...
		for (auto i = 0; i < width; i++)
		{
			...
			auto const R = (Y * yuv2rgb[0] + U * yuv2rgb[1] + V * yuv2rgb[2]);
			auto const G = (Y * yuv2rgb[3] + U * yuv2rgb[4] + V * yuv2rgb[5]);
			auto const B = (Y * yuv2rgb[6] + U * yuv2rgb[7] + V * yuv2rgb[8]);
			...
			newR = CLAMP_VALUE(newR, f32_value_black, f32_value_white);
			newG = CLAMP_VALUE(newG, f32_value_black, f32_value_white);
			newB = CLAMP_VALUE(newB, f32_value_black, f32_value_white);

			finalPixel.A = A;
			finalPixel.Y = newR * rgb2yuv[0] + newG * rgb2yuv[1] + newB * rgb2yuv[2];
			finalPixel.U = newR * rgb2yuv[3] + newG * rgb2yuv[4] + newB * rgb2yuv[5];
			finalPixel.V = newR * rgb2yuv[6] + newG * rgb2yuv[7] + newB * rgb2yuv[8];
...
) noexcept
{
	...
	for (auto j = 0; j < height; j++)
	{
		...
		for (auto i = 0; i < width; i++)
		{
			...
			/* convert sRGB to HSL format */
			sRgb2hsi(R, G, B, hue, saturation, value);
			...
			/* back convert to sRGB space */
			hsi2sRgb(newHue, newSat, newVal, newR, newG, newB);
