...
# -*- coding: utf-8 -*-
"""
gen_cct_lut_header_cpp20.py  (Python 3.12)

Generate a C++20 header containing the Planckian-locus CCT LUT as an
inline constexpr std::array, for one CIE standard observer (1931 2-deg
or 1964 10-deg), from an official CIE color-matching-function CSV file.

The NUMERIC CONTENT is bit-identical to the C++14 generator's output
(same doubles, same repr round-trip emission). Only the C++ language
level of the wrapper differs:
  - inline constexpr  : one shared table across all translation units
                        (C++14 namespace-scope constexpr is only implicitly
                        internal-linkage; every TU got its own copy).
  - designated initializers on the rows (self-documenting fields).
  - std::span view() accessor for pointer+size-free consumption.
...
def main() -> None:
    ...
    ap.add_argument("--positional", action="store_true",
                    help="emit positional row initializers (smaller file) instead of designated ones")
    ...
    out_path = args.out or (tag + "_CPP20.hpp")
    ...
    with open(out_path, "w", newline="\n") as f:
        ...
        w(" * Standard       : C++20 (inline constexpr, designated\n")
        w(" *                  initializers, std::span). Numeric content is\n")
        w(" *                  bit-identical to the C++14 generator output.\n")
        w(" */\n\n")
        ...
        # header (cpp14 AND cpp20), so all tables use ONE C++ type regardless
        # of which language variant the aggregator selects.
        ...
        w(f"    inline constexpr double CCT_MIN  = {args.cct_min!r};\n")
        w(f"    inline constexpr double CCT_MAX  = {args.cct_max!r};\n")
        w(f"    inline constexpr double CCT_STEP = {args.cct_step!r};\n\n")
        w(f"    inline constexpr std::size_t {tag}_SIZE = {count}u;\n\n")
        w(f"    inline constexpr std::array<CctLutRow_double, {tag}_SIZE> "
          f"{tag} =\n")
        w("    { {\n")
        if args.positional:
            ...
            for (T, u, v) in entries:
                w(f"        {{ .cct = {T!r}, .u = {u!r}, "
                  f".v = {v!r}, .Duv = 0.0 }},\n")
        w("    } };\n\n")
        w(f"    [[nodiscard]] constexpr std::span<const CctLutRow_double> "
          f"view() noexcept\n")
        w("    {\n")
        w(f"        return {{ {tag} }};\n")
        w("    }\n\n")
