	...
	PF_LayerDef		*output)
{
	PF_SPRINTF(
		...
		"%s, v%d.%d\r%s",
		strName,
		ImageSharpening_VersionMajor,
		ImageSharpening_VersionMinor,
		strCopyright);
	...
	PF_LayerDef		*output)
{
	...
	out_data->my_version =
		PF_VERSION(
			ImageSharpening_VersionMajor,
			ImageSharpening_VersionMinor,
			ImageSharpening_VersionSub,
			ImageSharpening_VersionStage,
			ImageSharpening_VersionBuild
		);
...
{
    return ImageSharpening_SmartRender (in_data, out_data, extraP);
}
...
{
    return ImageSharpening_PreRender (in_data, out_data, extraP);
}
