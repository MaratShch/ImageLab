...
)
{
	...
	for (j = 0; j < height; j++)
	{
		for (i = 0; i < width; i++)
		{
			...
			pDst[idx    ] = (pSrc[idx    ] * level_image + noiseB * level_noise) / 100.f;
			pDst[idx + 1] = (pSrc[idx + 1] * level_image + noiseG * level_noise) / 100.f;
			pDst[idx + 2] = (pSrc[idx + 2] * level_image + noiseR * level_noise) / 100.f;
			pDst[idx + 3] = (0 == noiseOnAlpha ?
				pSrc[idx + 3] : ((pSrc[idx + 3] * level_image + (static_cast<float>(romuTrio32_random()) / fDiv) * level_noise) / 100.f));
		}
	}
