...
/*
	INPUT RANGES  [floating point] -> H: 0...360  S,L : 0...100
	OUTPUT RANGES [fixed point]    -> R,G,B : 0...1023
*/
...
)
{
	...
	for (j = 0; j < height; j++)
	{
		...
		for (i = 0; i < width; i++)
		{
			...
			if (0.0f == S)
			{
				R = G = B = CLAMP_RGB10(static_cast<csSDK_int32>(L * 1024.0f));
			} /* if (0.0f == S) */
			else
			{
				tmpVal2 = (L < 0.50f) ? (L * (1.0f + S)) : (L + S - (L * S));
				...
				R = CLAMP_RGB10(static_cast<csSDK_int32>(fR * 1024.0f));
				G = CLAMP_RGB10(static_cast<csSDK_int32>(fG * 1024.0f));
				B = CLAMP_RGB10(static_cast<csSDK_int32>(fB * 1024.0f));
			}

			dstLine[i] = (B << 2) | (G << 12) | (R << 22);
