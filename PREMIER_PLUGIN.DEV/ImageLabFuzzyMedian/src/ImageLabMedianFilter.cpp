...
bool median_filter_BGRA_4444_8u_frame (const VideoHandle theData, const csSDK_int32& kernelWidth)
{
	...
	const csSDK_int32 medianPosition = (kernelWidth * kernelWidth) / 2 + 1;
	...
	for (j = border; j < lastLine; j++)
	{
		for (i = border; i < lastPixel; i++)
		{
			...
			for (l = lMin; l <= lMax; l++)
			{
				for (m = mMin; m <= mMax; m++)
				{
					const csSDK_uint32 inPixel = srcPix[l * linePitch + m];

					windowB[k] = (inPixel & 0x000000FFu);
					windowG[k] = (inPixel & 0x0000FF00u) >> 8;
					windowR[k] = (inPixel & 0x00FF0000u) >> 16;
