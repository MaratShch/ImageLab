...
#ifndef OFFSET_A32f
 #define OFFSET_A32f(idx) ((idx) * 4)
#endif

#ifndef OFFSET_R32f
 #define OFFSET_R32f(idx) ((idx) * 4 + 1)
#endif

#ifndef OFFSET_G32f
 #define OFFSET_G32f(idx) ((idx) * 4 + 2)
#endif

#ifndef OFFSET_B32f
 #define OFFSET_B32f(idx) ((idx) * 4 + 3)
#endif


static inline void process_ARGB_4444_32f_buffer
(
...
)
{
	...
	for (j = 0; j < height; j++)
	{
		...
		for (i = 0; i < width; i++)
		{
			...
			pDst[origIdx    ] = A;		/* copy A value from SRC */	
			pDst[origIdx + 1] = CLAMP_32F(R);
			pDst[origIdx + 2] = CLAMP_32F(G);
			pDst[origIdx + 3] = CLAMP_32F(B);
		}
	}
...
)
{
	...
	do
	{
		if (0 == iterCnt)
		{
			process_ARGB_4444_32f_buffer (pSrc, pBuffers[ping], width, height, linePitch, fNoiseLevel, currentTimeStep);
		}
		...
		{
			process_ARGB_4444_32f_buffer (pSrc, pBuffers[ping], pDst, width, height, linePitch, fNoiseLevel, currentTimeStep);
		}
